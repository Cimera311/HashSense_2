     ...
     globalThis.cancelExport = false;
      ...
      // Konfigurierbare Wartezeiten (in Millisekunden)
      globalThis.exportWaitTimes = {
        buttonWait: 5000,        // Wartezeit wenn Button nicht bereit
        pageBreakEvery: 10,      // Pause nach X Seiten
        pageBreakWait: 5000,     // Wartezeit bei Pause
        normalPageWait: 350,     // Normale Wartezeit zwischen Seiten
        unchangedPageWait: 5000  // Wartezeit wenn Seite unverändert
      };
      
      // Hilfsfunktion zum Ändern der Wartezeiten
      globalThis.setWaitTimes = function(settings) {
        Object.assign(globalThis.exportWaitTimes, settings);
        console.log('⚙️ Wartezeiten aktualisiert:', globalThis.exportWaitTimes);
      };
        ...
        async function goMiningAutoExport() {
          ...
          while (true) {
            ...
            if (!nextButton || nextButton.disabled) {
              console.log(`Button not ready yet. Waiting ${exportWaitTimes.buttonWait}ms and trying again...`);
              await new Promise(r => setTimeout(r, exportWaitTimes.buttonWait));
              icon = document.querySelector('icon-arrow-short[direction="right"]');
              nextButton = icon?.closest('button');

              if (!nextButton || nextButton.disabled) {
                console.log(`Button still not ready. Waiting ${exportWaitTimes.buttonWait}ms again...`);
                await new Promise(r => setTimeout(r, exportWaitTimes.buttonWait));
                icon = document.querySelector('icon-arrow-short[direction="right"]');
                nextButton = icon?.closest('button');
            ...
            nextButton.click();
            if (pageCounter % exportWaitTimes.pageBreakEvery === 0) {
              console.log(`Short break after page ${pageCounter} (${exportWaitTimes.pageBreakWait}ms)...`);
              await new Promise(r => setTimeout(r, exportWaitTimes.pageBreakWait));
            } else {
              await new Promise(r => setTimeout(r, exportWaitTimes.normalPageWait));
            }

            let newKey = getFirstRowKey();
            if (newKey === previousKey) {
              console.log(`Page appears unchanged. Waiting ${exportWaitTimes.unchangedPageWait}ms and checking again...`);
              await new Promise(r => setTimeout(r, exportWaitTimes.unchangedPageWait));
